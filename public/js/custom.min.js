$(function() {
    ...
    var i = function() {
        (window.innerWidth > 0 ? window.innerWidth : this.screen.width) < 70 ? ($("body").addClass("mini-sidebar"),
    ...
    $(window).ready(i), $(window).on("resize", i), $(".sidebartoggler").on("click", function() {
            $("body").hasClass("mini-sidebar") ? ($("body").trigger("resize"), $(".scroll-sidebar, .slimScrollDiv").css("overflow", "hidden").parent().css("overflow", "visible"),
                $("body").removeClass("mini-sidebar"), $(".navbar-brand span").show(), $(".profSidebar").show()) : ($("body").trigger("resize"),
                $(".scroll-sidebar, .slimScrollDiv").css("overflow-x", "visible").parent().css("overflow", "visible"),
                $("body").addClass("mini-sidebar"), $(".navbar-brand span").hide(), $(".profSidebar").hide());
        }),
...
var loadEvaluation = function(id, type, cb){
    $.get('api/v1/'+ type +'Evaluation/' + id, function(response){
...
var loadEval = function(type){
    switch(type){
        case 'inst' : {
            var id = $('.instNum').html();
            loadEvaluation(id, type, function(err, data){
                if(err) console.log("Error: " + err.status + " " + err.statusText);
                else renderInstEvaluation(data);
            });
            break;
        }
        case 'stud' : {
...
var createAcc = function(_type, _id, _user, _pass, cb){
    $.post('api/v1/' + _type, {id: _id, username: _user, password: _pass}, function(response){
        if(response.error) return cb(response.detail);
        cb(null, response.data);
    });
}